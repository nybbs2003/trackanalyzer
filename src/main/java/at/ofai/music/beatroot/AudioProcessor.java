...
public class AudioProcessor {
	...
	/**
	 * Uncompressed version of <code>rawInputStream</code>. In the (normal) case where the input is already PCM data, <code>rawInputStream == pcmInputStream</code>
	 */
	...
	/**
	 * Spacing of audio frames (determines the amount of overlap or skip between frames). This value is expressed in seconds and can be set by the command line option <b>-h hopTime</b>. (Default =
	 * 0.020s)
	 */
	protected double hopTime;

	/**
	 * The approximate size of an FFT frame in seconds, as set by the command line option <b>-f FFTTime</b>. (Default = 0.04644s). The value is adjusted so that <code>fftSize</code> is always power of
	 * 2.
	 */
	...
	/**
	 * A mapping function for mapping FFT bins to final frequency bins. The mapping is linear (1-1) until the resolution reaches 2 points per semitone, then logarithmic with a semitone resolution.
	 * e.g. for 44.1kHz sampling rate and fftSize of 2048 (46ms), bin spacing is 21.5Hz, which is mapped linearly for bins 0-34 (0 to 732Hz), and logarithmically for the remaining bins (midi notes 79
	 * to 127, bins 35 to 83), where all energy above note 127 is mapped into the final bin.
	 */
	protected int[] freqMap;

	/**
	 * The number of entries in <code>freqMap</code>. Note that the length of the array is greater, because its size is not known at creation time.
	 */
	...
	/**
	 * Constructor: note that streams are not opened until the input file is set (see <code>setInputFile()</code>).
	 */
	...
	public String toString() {
		return "AudioProcessor\n" + String.format("\tFile: %s (%3.1f kHz, %1d channels)\n", audioFileName, sampleRate / 1000, channels) + String.format("\tHop / FFT sizes: %5.3f / %5.3f", hopTime, hopTime * fftSize / hopSize);
	} // toString()
	...
	/**
	 * Sets up the streams and buffers for live audio input (CD quality). If any Exception is thrown within this method, it is caught, and any opened streams are closed, and
	 * <code>pcmInputStream</code> is set to <code>null</code>, indicating that the method did not complete successfully.
	 */
	...
	/**
	 * Sets up the streams and buffers for audio file input. If any Exception is thrown within this method, it is caught, and any opened streams are closed, and <code>pcmInputStream</code> is set to
	 * <code>null</code>, indicating that the method did not complete successfully.
	 * 
	 * @param fileName
	...
	public void setInputFile(String fileName) {
		...
		try {
			...
			if ((audioFormat.getEncoding() != AudioFormat.Encoding.PCM_SIGNED) || (audioFormat.getFrameSize() != channels * 2) || audioFormat.isBigEndian()) {
				AudioFormat desiredFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, sampleRate, 16, channels, channels * 2, sampleRate, false);
	...
	/**
	 * Creates a map of FFT frequency bins to comparison bins. Where the spacing of FFT bins is less than 0.5 semitones, the mapping is one to one. Where the spacing is greater than 0.5 semitones, the
	 * FFT energy is mapped into semitone-wide bins. No scaling is performed; that is the energy is summed into the comparison bins. See also processFrame()
	 */
	 ...
	 * 
	 * @return true if a frame (or part of a frame, if it is the final frame) is read. If a complete frame cannot be read, the InputStream is set to null.
	 */
	...
	/**
	 * Processes a frame of audio data by first computing the STFT with a Hamming window, then mapping the frequency bins into a part-linear part-logarithmic array, then computing the spectral flux
	...
	protected void processFrame() {
		if (getFrame()) {
			...
			for (int j = 0; j < energyOversampleFactor; j++) {
				...
				energy[frameCount * energyOversampleFactor + j] = newEnergy / sz <= 1e-6 ? 0 : Math.log(newEnergy / sz) + 13.816;
			}
			double decay = frameCount >= 200 ? 0.99 : (frameCount < 100 ? 0 : (frameCount - 100) / 100.0);
			if (ltAverage == 0)
				ltAverage = frameRMS;
			else
	...
	/**
	 * Reads a file of feature values, treated as an onset detection function, and finds peaks, which are stored in <code>onsetList</code> and <code>onsets</code>.
	 * 
	 * @param fileName
	 *            The file of feature values
	 * @param hopTime
