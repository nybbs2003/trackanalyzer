...
public class FFT {
	...
	/**
	 * used in {@link FFT#fft(double[], double[], int)} to specify a forward Fourier transform
	 */
	public static final int FORWARD = -1;
	/**
	 * used in {@link FFT#fft(double[], double[], int)} to specify an inverse Fourier transform
	 */
	public static final int REVERSE = 1;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a rectangular window function
	 */
	public static final int RECT = 0;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a Hamming window function
	 */
	public static final int HAMMING = 1;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a 61-dB 3-sample Blackman-Harris window function
	 */
	public static final int BH3 = 2;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a 74-dB 4-sample Blackman-Harris window function
	 */
	public static final int BH4 = 3;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a minimum 3-sample Blackman-Harris window function
	 */
	public static final int BH3MIN = 4;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a minimum 4-sample Blackman-Harris window function
	 */
	public static final int BH4MIN = 5;
	/**
	 * used in {@link FFT#makeWindow(int,int,int)} to specify a Gaussian window function
	 */
	 ...
	 * 
	 * @param re
	 *            the real part of the input data and the power of the output data
	 * @param im
	...
	/**
	 * Computes a real FFT and converts the results to polar coordinates (power and phase). Both arrays must be the same length, which is a power of 2. This is a optimized version which doesn't return
	 * phase. Be careful!
	 * 
	 * @param re
	 *            the real part of the input data and the power of the output data
	 * @param im
	 *            the imaginary part of the input data (=0,..,0) and the phase of the output data
	 */
	 ...
	 * 
	 * @param pow
	 *            the power of the spectral input data (and real part of the output data)
	 * @param ph
	...
	/**
	 * Computes a real FFT and converts the results to polar coordinates (magnitude and phase). Both arrays must be the same length, which is a power of 2.
	 * 
	 * @param re
	 *            the real part of the input data and the magnitude of the output data
	 * @param im
	 *            the imaginary part of the input data and the phase of the output data (assumed to be 0,...,0)
	 */
	 ...
	 * @param size
	 *            the number of non zero values; if the array is larger than this, it is zero-padded symmetrically at both ends
	 */
	...
	static void blackmanHarris4sMin(double[] data, int size) {
		...
		for (int i = 0; start < stop; start++, i++)
			data[i] = scale * (0.35875 - 0.48829 * Math.cos(twoPI * i / size) + 0.14128 * Math.cos(2 * twoPI * i / size) - 0.01168 * Math.cos(3 * twoPI * i / size));
	} // blackmanHarris4sMin()
	...
	static void blackmanHarris4s(double[] data, int size) {
		...
		for (int i = 0; start < stop; start++, i++)
			data[i] = scale * (0.40217 - 0.49703 * Math.cos(twoPI * i / size) + 0.09392 * Math.cos(2 * twoPI * i / size) - 0.00183 * Math.cos(3 * twoPI * i / size));
	} // blackmanHarris4s()
	...
	static void blackmanHarris3sMin(double[] data, int size) {
		...
		for (int i = 0; start < stop; start++, i++)
			data[i] = scale * (0.42323 - 0.49755 * Math.cos(twoPI * i / size) + 0.07922 * Math.cos(2 * twoPI * i / size));
	} // blackmanHarris3sMin()
	...
	static void blackmanHarris3s(double[] data, int size) {
		...
		for (int i = 0; start < stop; start++, i++)
			data[i] = scale * (0.44959 - 0.49364 * Math.cos(twoPI * i / size) + 0.05677 * Math.cos(2 * twoPI * i / size));
	} // blackmanHarris3s()
	 ...
	 * information on characteristics of these functions can be found in F.J. Harris (1978), On the Use of Windows for Harmonic Analysis with the Discrete Fourier Transform, <em>Proceedings of the
	 * IEEE</em>, 66, 1, 51-83.
	 * 
	 * @param choice
	 *            the choice of window function, one of the constants defined above
	 * @param size
	 *            the size of the returned array
	 * @param support
	 *            the number of non-zero values in the array
	 ...
	 * 
	 * @param data
	 *            the array of input data, also used for output
	 * @param window
